let charts = {};
let currentPDFIndex = 0; // Índice do PDF atual
let currentPDFs = [];    // Lista de PDFs da área selecionada

// Mapear área para PDFs correspondentes
const pdfMap = {
    "GAD": [
...
// Função para abrir card RNC com PDF e navegação
function abrirCardRNC(area) {
    const card = document.getElementById("cardRNC");
    const viewer = document.getElementById("pdfViewer");

    currentPDFs = pdfMap[area] || [];
    currentPDFIndex = 0;

    if(currentPDFs.length === 0) {
        viewer.innerHTML = `
            <div style="padding:20px; text-align:center; font-size:16px; color:#555;">
                ❌ Nenhum PDF disponível para esta área.
            </div>
        `;
    } else {
        viewer.innerHTML = `
            <div style="text-align:center; margin-bottom:10px;">
                <button id="prevPDF" style="margin-right:10px;">⬅ Anterior</button>
                <button id="nextPDF">Próximo ➡</button>
            </div>
            <iframe id="pdfFrame" src="${currentPDFs[currentPDFIndex]}" style="width:100%; height:400px; border:none;"></iframe>
            <div style="margin-top:5px; font-size:14px; color:#555;">Área: ${area} (${currentPDFIndex+1} de ${currentPDFs.length})</div>
        `;

        document.getElementById("prevPDF").addEventListener('click', () => {
            if(currentPDFIndex > 0) currentPDFIndex--;
            atualizarPDFViewer(area);
        });

        document.getElementById("nextPDF").addEventListener('click', () => {
            if(currentPDFIndex < currentPDFs.length - 1) currentPDFIndex++;
            atualizarPDFViewer(area);
        });
    }
...
function atualizarPDFViewer(area) {
    const pdfFrame = document.getElementById("pdfFrame");
    const info = pdfFrame.nextElementSibling;
    if(pdfFrame) {
        pdfFrame.src = currentPDFs[currentPDFIndex];
        info.innerText = `Área: ${area} (${currentPDFIndex+1} de ${currentPDFs.length})`;
    }
}
